...
from nova.planner import Plan, PlanStep, RecoveryPolicy, determine_required_skills
    ...
    def _execute_plan_thread(self, plan_id: str) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                emit("task_completed", module="execution_engine", status="success" if plan.status == ExecutionState.COMPLETED else "failed", metadata={
                    "plan_id": plan_id,
                    "status": plan.status,
...
def route_query_to_planner_pipeline(
    ...
    """
    from nova.planner import Planner, Goal
