...
try:
    ...
    from nova.voice.pipeline import run_voice_loop, VoiceState, _current_state, shutdown_event
...
class TestLifecycleSafeguards(unittest.TestCase):
    ...
    def test_voice_thread_stability_under_exceptions(
        ...
        with patch("nova.voice.pipeline.AudioQualityAnalyzer") as mock_qa_class:
            mock_qa = MagicMock()
            ...
            from nova.voice.pipeline import get_current_state
    ...
    def test_voice_loop_integration_iteration(
        ...
        from nova.voice.pipeline import run_voice_loop, VoiceState, get_current_state
        import threading
        ...
        with patch("nova.voice.pipeline.shutdown_event", local_shutdown), \
             patch("nova.voice.pipeline.voice_active_event", local_voice_active), \
             patch("time.sleep", return_value=None), \
             patch("nova.voice.pipeline.record_audio_from_stream", return_value=b"dummy audio"), \
